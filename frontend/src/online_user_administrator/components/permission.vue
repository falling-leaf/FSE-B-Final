      ...
      <el-table-column fixed="right" label="Operations" width="150">
        <template #default="scope">
          ...
          <el-button
            ...
            @click="
              (LostVisible = true),
                ((Lost_id = scope.row.user_id), (UnLost_id = scope.row.user_id))
            "
            >Lost</el-button
          >
        </template>
...
export default {
  ...
  methods: {
    async QueryUserData() {
      let response = await axios.get("/OBmanager/user_data_query/"); // 向/book发出GET请求s
    ...
    async Lost() {
      ...
      // console.log("7878", this.Lost_id);
      axios
        .post("/OBmanager/user_lost/", {
